#!/usr/bin/env python
# filename:     ${SCR_DIR}/clweather.py
# purpose:      show weather forecast with Unicode symbols in terminal
# author:       (c) Alexander Puls
# bug reports:  see https://github.com/0vv1
# license:      This program is licensed under the GPL v3.
######################################################################
# note:         Needs a font patched with Nerd Font patches to show  
#               the correct Unicode symbols of the Nerd Font  
#               nf-weather group. See https://nerdfonts.com/#home.
######################################################################
...
def main():
    ...
    if forecast['cod'] == 400 or forecast['cod'] == 401:
        ...
        # Unicode symbol assignment depending on API condition IDs and daytime
        else:
            ...
            if args.place:
                if args.imperial:
                    print(symbol + ' ' + tempForecast + u'\u00b0' + 'F in ' + cityForecast)
                else:
                    print(symbol + ' ' + tempForecast + u'\u00b0' + 'C in ' + cityForecast)
            else:
                if args.imperial:
                    print(symbol + tempForecast + u'\ue341')
                else:
                    print(symbol + tempForecast + u'\ue339')
